    ...
    ,{
      ...
      .then(function(myJson) {
        if(type == "all"){
          ...
          const CustomerCategory = myJson.filter((value) => value.category == type)
...
const [selectedOption, setSelectedOption] = useState([]);
  ...
  const selectOp = (selected,option) => {
    ...
    selected.forEach((item) => {
      const temp = {"custName":item.custName,"id":item.id,"category":item.category}
      arr.push(temp)
    })
  ...
  const customStyles = {
    ...
    multiValue: (style,{data}) => ({
      ...
      backgroundColor: data.category == "driver"
      ? "#E3FFF9"
      : "#F0F0F0",
      color: data.category == "diver"
      ? "#44B49C"
      : "black",
...
return (
  ...
  <div className="BroadcastBloc-tabs" style={{height:"40px"}}>
        ...
        <button className={toggleBroadcastState === 2 ? "broadCastTabs active-tabs" : "broadCastTabs"} onClick={() => {toggleTab(2,"client")}}>
          Clients
        </button>

        <button className={toggleBroadcastState === 3 ? "broadCastTabs active-tabs" : "broadCastTabs"} onClick={() =>{toggleTab(3,"driver")}}>
          Drivers
        </button>
...
// Options List...........................
  const MyOption = (props) => {
    ...
    return (
      <>
      <div className='optionsDiv'  ref={innerRef} {...innerProps} style={{cursor:"pointer",height:"60px",width:"340px",padding:"10px",fontFamily:"Noto Sans",display: "flex",alignItems:"center" }}>
          <img src={props.data.image} alt="" />
          <div style={{marginLeft:"5px",fontSize:"19px",margin:"0px 10px"}}>{props.data.custName}</div>
          <div style={props.data.category == "driver" ? {fontSize:"10px",padding:"2px 8px",borderRadius:"4px",color:"#44B39B",backgroundColor:"#E2FEF8"}: {padding:"2px 8px",fontSize:"10px",color:"#373737",backgroundColor:"#F0F0F0"}}>
            {props.data.category}
          </div>
      </div>
      </>
        ...
        <div className="toBroadCast">
          ...
          <div className="selectDiv">
            <Select
                  ...
                  styles={customStyles}  
                  onChange={(selected,option) => selectOp(selected,option)}
                  components={{MenuList:CustomMenu,Option:MyOption,MultiValueContainer,DropdownIndicator:() => null, IndicatorSeparator:() => null}}
                  isMulti={true}/>
          </div>
        </div>
